...
const DashboardPage: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemAnimation} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <StatCard
          title="Total Balance"
      ...
      <motion.div variants={itemAnimation}>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/transactions" className="text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300 text-sm font-medium flex items-center">
            View All <ListChecks size={16} className="ml-1"/>
          </Link>
        </div>
        <TransactionTable
          transactions={recentTransactions}
          onEditTransaction={() => {navigate('/transactions', { state: { edit: true } });
          }}
        />
      </motion.div>
    </motion.div>
